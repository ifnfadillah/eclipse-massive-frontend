  ...
  const webinars = [
    {
      ...
      link: "https://docs.google.com/forms/d/e/1FAIpQLSeEBJo8azqIMyLfP1siFcbe24iSCb2Yd7IMRlg5Zvyo157bJw/viewform?usp=sharing",
    },
    {
      id: 2,
      imageSrc: "/assets/article1.jpg",
      ...
      <div className="w-full mt-5 sm:px-8 py-10 flex flex-col items-center gap-12">
        <div className="flex flex-col md:flex-row items-center px-8">
          <div className="w-full md:w-1/3 mb-4 md:mb-0">
            <img src={webinar.imageSrc} alt={webinar.title} className="w-full h-auto" />
          </div>
          <div className="w-full md:w-2/3 md:px-24 space-y-4 md:space-y-8 font-primary ">
            <h2 className="text-2xl md:text-4xl font-bold">{webinar.title}</h2>
            <p className="text-gray-700 text-xs md:text-sm">{webinar.description}</p>
            <h3 className="text-sm font-medium">
              Penyelenggara: <span className="text-sm font-normal">{webinar.author}</span>
            </h3>
            <h3 className="text-sm font-medium">
              Tanggal: <span className="text-sm font-normal">{webinar.date}</span>
            </h3>
            <h3 className="text-sm font-medium">
              Pukul: <span className="text-sm font-normal">{webinar.time}</span>
            </h3>
            <h3 className="text-sm font-medium">
              Harga: <span className="text-sm font-normal">{webinar.price}</span>
            </h3>
            <button className="w-full inline-flex items-center justify-center md:w-auto md:px-5 py-3 font-primary text-sm font-medium bg-sky-700 rounded-lg md:rounded-xl shadow border-2 text-white hover:bg-sky-900" type="" name="daftar">
              <Link to={webinar.link} target="_blank" rel="noopener noreferrer" className="flex items-center">
                Daftar Sekarang
        ...
        <div className="sm:py-16">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-y-8">
            {displayedWebinars.map((webinar) => (
              <Link key={webinar.id} to={`/sharenting-webinar/${webinar.id}`} className="mx-2">
